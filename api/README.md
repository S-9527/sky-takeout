...
| `sky-notify-api.json` | 支付回调接口 (com.sky.controller.notify) | 1 |

重新生成方式（后端须运行在 8080）：

```bash
curl -s -G "http://localhost:8080/v2/api-docs" --data-urlencode "group=管理端接口" -o sky-admin-api.json
curl -s -G "http://localhost:8080/v2/api-docs" --data-urlencode "group=用户端接口" -o sky-user-api.json
curl -s -G "http://localhost:8080/v2/api-docs" --data-urlencode "group=支付回调接口" -o sky-notify-api.json
```
